...
func (m *MockExporter) Start() error {
	...
	cmd := fmt.Sprintf("docker run --rm -itd --privileged --name %v %v %v -e SIM_ENABLE=1 -e PATH=$PATH:/home/amd/bin/ %v", m.Name, strings.Join(portsExposed, " "), mountOps, m.ImageURL)
	log.Print(cmd)
