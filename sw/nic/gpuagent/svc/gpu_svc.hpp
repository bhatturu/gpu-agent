...
aga_svc_gpu_get (const GPUGetRequest *proto_req, GPUGetResponse *proto_rsp)
{
    ...
    // an unset mask (0) means the default groups (no expensive PROCESS walk)
    info_mask = proto_req->infomask();
    if (info_mask == 0) {
        info_mask = AGA_GPU_INFO_DEFAULT;
    }
    if (proto_req->id_size() == 0) {
        ret = aga_gpu_read_all(aga_gpu_api_info_to_proto, proto_rsp, info_mask);
    ...
    for (int i = 0; i < proto_req->id_size(); i ++) {
        ...
        ret = aga_gpu_read(&key, &info, info_mask);
